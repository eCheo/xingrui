            ...
            if (item.children) {
              item.children.forEach(it => {
                if (it.name !== '首页' || it.name !== '编辑项目' || it.name !== '新增项目' || it.name !== '客户详情' || it.name !== '修改密码' || it.name !== '项目详情') {
                  if (res.data.data.memberType === 'front' && (it.name === '共享池' || it.name === '项目' || it.name === '我的客户')) {
                    it.meta.invisible = false
                  } else if(res.data.data.memberType === 'admin' && (it.name === '员工管理' || it.name === '客户管理' || it.name === '项目管理' || it.name === '设置')) {
                    it.meta.invisible = false
                  } else if(res.data.data.memberType === 'backend' && (it.name === '员工管理' || it.name === '客户管理' || it.name === '项目管理' || it.name === '设置')) {
                    it.meta.invisible = false
                  }  else {
